...
bool UserStorage::loadFromFile(const char* fileName) {
    ...
    if (fileCount < 0 || fileCount > maxCount_) return false;
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        u.setProfile(id, name, login, gmail, password, drinks);
    }
...
int UserStorage::findByLogin(const char* login) const {
    ...
    for (int i = 0; i < count_; ++i) {
        if (std::strcmp(arr_[i].getLogin(), login) == 0)
            return i;
    }
...
bool UserStorage::registerUser(const User& user) {
    if (!arr_ || count_ >= maxCount_) return false;

    if (findByLogin(user.getLogin()) != -1)
        return false;
...
bool UserStorage::loginUser(const char* login, const char* password, User& foundUser) const {
    ...
    if (std::strcmp(arr_[pos].getPassword(), password) != 0)
        return false;
