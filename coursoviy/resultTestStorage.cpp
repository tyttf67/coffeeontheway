...
bool ResultStorage::saveToFile(const char* fileName) const {
    ...
    for (int i = 0; i < count_; ++i) { out.write(reinterpret_cast<const char*>(&arr_[i]), sizeof(Result)); if (!out.good()) return false; }
    return true;
}
...
bool ResultStorage::loadFromFile(const char* fileName) {
    ...
    for (int i = 0; i < countInFile; ++i) { in.read(reinterpret_cast<char*>(&arr_[i]), sizeof(Result)); if (!in.good()) return false; }
    count_ = countInFile;
    return true;
}
...
bool ResultStorage::addResult(const Result& result) { if (!arr_ || count_ < 0 || count_ >= maxCount_) return false; arr_[count_++] = result; return true; }
