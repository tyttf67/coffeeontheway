...
bool CoffeeStorage::saveToTwoFiles(const char* mainFile, const char* backupFile) const {
    return saveToFile(mainFile) && saveToFile(backupFile);
}

bool CoffeeStorage::loadFromAnyFile(const char* mainFile, const char* backupFile) {
